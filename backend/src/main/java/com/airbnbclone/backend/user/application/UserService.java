package com.airbnbclone.backend.user.application;

import com.airbnbclone.backend.infrastructure.config.SecurityUtils;
import com.airbnbclone.backend.user.application.dto.ReadUserDTO;
import com.airbnbclone.backend.user.domain.User;
import com.airbnbclone.backend.user.mapper.UserMapper;
import com.airbnbclone.backend.user.repository.UserRepository;
...
public class UserService {
  ...
  public ReadUserDTO getAuthenticatedUserFromSecurityContext() {
    OAuth2User principal = (OAuth2User) SecurityContextHolder
      ...
      .getAuthentication()
      .getPrincipal();
    User user = SecurityUtils.mapOauth2AttibutesToUser(
  ...
  public void syncWithIdp(OAuth2User oAuth2User, boolean forceResync) {
    ...
    User user = SecurityUtils.mapOauth2AttibutesToUser(attributes);
    Optional<User> existingUser = userRepository.findOneByEmail(
      user.getEmail()
    );
    if (existingUser.isPresent()) {
      if (attributes.get(UPDATED_AT_KEY) != null) {
        Instant lastModifiedDate = existingUser
          .orElseThrow()
          .getLastModifiedDate();
        ...
        if (attributes.get(UPDATED_AT_KEY) instanceof Instant instant) {
          ...
          idpModifiedDate =
            Instant.ofEpochSecond((Integer) attributes.get(UPDATED_AT_KEY));
        }
  ...
  private void updateUser(User user) {
    Optional<User> userToUpdateOpt = userRepository.findOneByEmail(
      (user.getEmail())
    );
