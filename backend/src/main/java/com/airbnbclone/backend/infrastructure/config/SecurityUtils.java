...
public class SecurityUtils {
  ...
  public static final String CLAIMS_NAMESPACE =
    "https://ishaanvadhan.netlify.app/roles";

  public static User mapOauth2AttibutesToUser(Map<String, Object> attributes) {
    User user = new User();
    ...
    if (attributes.get("email") != null) {
      ...
      sub.contains("|") && username != null && username.contains("@")
    ) {
    ...
    if (attributes.get(CLAIMS_NAMESPACE) != null) {
      ...
      List<String> authoritiesRaw = (List<String>) attributes.get(
        CLAIMS_NAMESPACE
      );
      Set<Authority> authorities = authoritiesRaw
        .stream()
        .map(authority -> {
  ...
  public static List<SimpleGrantedAuthority> extractAuthorityFromClaims(
    Map<String, Object> claims
  ) {
    return mapRolesTOGrantedAuthorities(getRolesFromClaims(claims));
  }

  private static Collection<String> getRolesFromClaims(
    Map<String, Object> claims
  ) {
    @SuppressWarnings("unchecked")
    List<String> roles = (List<String>) claims.get(CLAIMS_NAMESPACE);
    return roles;
  }

  private static List<SimpleGrantedAuthority> mapRolesTOGrantedAuthorities(
